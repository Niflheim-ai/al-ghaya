...
// Determine if published status (best is to fetch $program and pass to the chapter page)
...
<section class="content-section">
    ...
    <div class="w-full h-fit flex flex-col bg-company_white gap-[20px] p-[20px] rounded-[40px] items-start justify-start mb-8">
        ...
        <?php if ($isPublished): ?>
            <div class="mb-5 px-4 py-3 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-900 rounded-lg w-full">
                <b>This chapter belongs to a published program and cannot be edited.</b><br>
                Create a new update draft to modify content.
            </div>
        <?php endif; ?>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea name="content" rows="5"
                              class="w-full px-4 py-3 border border-gray-300 rounded-[10px] focus:ring-2 focus:ring-company_blue focus:border-company_blue"
                              <?= $isPublished ? 'disabled' : '' ?>><?= htmlspecialchars($chapter['content'] ?? '') ?></textarea>
                </div>
            </div>
    ...
    <div class="w-full h-fit flex flex-col bg-company_white gap-[20px] p-[20px] rounded-[40px] items-start justify-start">
        <div class="w-full flex gap-[25px] items-center justify-between">
            ...
            <?php if (count($stories) < 3 && !$isPublished): ?>
                    ...
                    <div class="story-card bg-white border border-gray-200 rounded-[20px] shadow-sm hover:shadow-lg transition-all duration-200">
                        ...
                        <div class="p-[20px]">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                <!-- Story action: always show (eye if published, pencil if draft) -->
                                ...
                                title="<?= $isPublished ? 'View Story (published, read-only)' : 'Edit Story' ?>">
                                    <?php if ($isPublished): ?>
                                        <i class="ph ph-eye"></i> View Story
                                    <?php else: ?>
    ...
    <!-- Quiz Section (locked if published) -->
    <?php $quiz = getChapterQuiz($conn, $chapterId); ?>
    <div class="w-full h-fit flex flex-col bg-company_white gap-[20px] p-[20px] rounded-[40px] items-start justify-start mt-8">
        <div class="w-full flex gap-[25px] items-center justify-between">
            ...
            class="px-4 py-2 <?= $isPublished ? 'bg-gray-400' : 'bg-company_orange hover:opacity-90' ?> text-white rounded-[10px] transition-opacity inline-flex items-center gap-2"
            title="<?= $isPublished ? 'View Quiz (published, read-only)' : ($quiz ? 'Edit Quiz' : 'Add Quiz') ?>">
                <?php if ($isPublished): ?>
                    <i class="ph ph-eye"></i>
                    <span class="body-text3-semibold">View Quiz</span>
                <?php else: ?>
        ...
        <?php else: ?>
            <!-- Quiz Display (readonly if published) -->
